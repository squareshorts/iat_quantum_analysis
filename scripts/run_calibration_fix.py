...
def step1_low_angle_recovery(x6, mask6, y6_std):
    ...
    raw_df.to_csv(OUT_DIR / "calib_recovery_raw.csv", index=False)
    ...
    summ_df = (raw_df.groupby("theta_true")
                     ...
                     .reset_index())
    summ_df.to_csv(OUT_DIR / "calib_recovery_summary.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "calib_recovery_lowangle.png", dpi=300)
    plt.close()
...
def step2_n_scaling(x6, mask6, y6_std):
    print(f"\n[STEP 2] N-scaling at theta=17.25°: "
    ...
    rows.append({
        ...
        "hdi_low": 16.50, "hdi_high": 18.25,
        "bias": 0.0, "covered": True,
    })

    df = pd.DataFrame(rows)
    df.to_csv(OUT_DIR / "calib_nscaling.csv", index=False)
    ...
    agg.to_csv(OUT_DIR / "calib_nscaling_summary.csv", index=False)
    ...
    ax1.scatter([141329], [17.25], color="crimson", s=100, zorder=5,
    ...
    ax2.scatter([141329], [1.0], color="crimson", s=100, zorder=5,
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "calib_nscaling.png", dpi=300)
    plt.close()
...
def step3_verdict(recovery_summ, nscaling_agg):
    ...
    criteria = {
        ...
        "mean_MAP within 10° of 17.25° at N=10000":
            bool(abs(n10k["mean_map"] - 17.25) < 10.0),
    ...
    if all_pass:
        ...
        print("    at population scale. 17.25° estimate is calibration-backed.")
    else:
...
def main():
    ...
    print(f"Recovery: {RECOVERY_REPS} reps x {len(LOW_ANGLE_THETAS)} angles, "
          f"N={SIM_N_CURVES}")
    print(f"N-scaling: {N_SCALE_REPS} reps x {len(N_SCALE_SIZES)} sizes at θ=17.25°")
    ...
    curves  = load_or_build_curves(df_raw, n_bins=PRIMARY_BINS, blockwise=False)
